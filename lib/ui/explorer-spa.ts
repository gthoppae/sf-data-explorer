...
import { formatValue, fit, isBackspaceKey, pad, stripAnsi, type ThemeLike, wrapPlain } from "../text.ts";
...
export class ExplorerSpa<TObject, TField> implements Component {
  ...
  private paneWeights: [number, number, number] = [0.30, 0.32, 0.38];
  ...
  handleInput(data: string): void {
    ...
    if (this.confirmQuit) {
      if (matchesKey(data, Key.enter) || matchesKey(data, Key.escape) || data === "q" || data.toLowerCase() === "y") return this.args.done();
    ...
    if (data === "m" && this.args.strategy.alternateCatalog) return void this.toggleAlternateCatalog();
    ...
    if (this.focus === "query" && (matchesKey(data, Key.enter) || data === " ")) return this.openDetail();
  }
  ...
  render(width: number): string[] {
    ...
    const pill = this.args.transportInfo ? t.fg("borderAccent", `[${transportLabel(this.args.transportInfo)}]`) : "";
    const ruleWidth = Math.max(0, w - visibleWidth(title) - (pill ? visibleWidth(pill) + 1 : 0));
    const lines = [`${t.fg("accent", t.bold(title))}${t.fg("border", "─".repeat(ruleWidth))}${pill ? ` ${pill}` : ""}`];
    ...
    const alt = this.args.strategy.alternateCatalog ? ` · m ${this.args.strategy.alternateCatalog.label}` : "";
    lines.push(fit(t.fg("dim", `? help · t switch · ←→ pane · ↑↓ move · / filter${filter}${alt} · enter select/detail · w ${this.args.strategy.whereLabel} · l ${this.args.strategy.limitLabel} · e edit query · b rebuild · r run · c copy · s save · f refresh · q close`), w));
    if (this.queryEditing) lines.push(fit(t.fg("accent", "Editing query: enter save · esc cancel · arrows/home/end move · paste supported"), w));
    else if (this.editing) {
      const prompt = this.editing === "where" ? this.args.strategy.whereLabel : this.args.strategy.limitLabel;
  ...
  private renderColumns(w: number): string[] {
    ...
    const leftSep = t.fg(this.focus === "objects" || this.focus === "fields" ? "accent" : "border", " │ ");
    const rightSep = t.fg(this.focus === "fields" || this.focus === "query" ? "accent" : "border", " │ ");
    ...
    const lines: string[] = [];
    for (let i = 0; i < rows; i += 1) lines.push(fit(pad(objectPane[i] ?? "", objectW) + leftSep + pad(fieldPane[i] ?? "", fieldW) + rightSep + fit(queryPane[i] ?? "", queryW), w));
    return lines;
  }
  ...
  private paneHeader(label: string, focused: boolean, width: number): string[] {
    ...
    return [fit(`${marker} ${title}`, width), t.fg(focused ? "accent" : "border", ruleChar.repeat(Math.max(0, width)))];
  }
  ...
  private renderObjects(width: number): string[] {
    ...
    const lines = this.paneHeader(`${this.args.strategy.objectKindLabel()} (${filtered.length}/${this.objects.length})`, this.focus === "objects", width);
    lines.push(fit(t.fg(this.objectCacheLine.startsWith("Serving") ? "success" : "warning", this.objectCacheLine), width));
    if (this.objectQuery || (this.searchMode && this.focus === "objects")) lines.push(fit(t.fg("accent", `/${this.objectQuery}`), width));
    ...
    for (let i = this.objectScrollTop; i < end; i += 1) {
      ...
      const active = this.selectedObject && this.args.strategy.objectName(obj) === this.args.strategy.objectName(this.selectedObject);
      if (this.args.strategy.objectRow) lines.push(...this.args.strategy.objectRow(obj, selected, !!active, width, t));
      else {
        const prefix = selected ? t.fg("accent", "› ") : active ? t.fg("success", "◆ ") : "  ";
        lines.push(fit(`${prefix}${selected ? t.fg("accent", this.args.strategy.objectDisplayName(obj)) : this.args.strategy.objectDisplayName(obj)}`, width));
  ...
  private renderFields(width: number): string[] {
    ...
    if (this.fieldQuery || (this.searchMode && this.focus === "fields")) lines.push(fit(t.fg("accent", `/${this.fieldQuery}`), width));
    ...
    for (let i = this.fieldScrollTop; i < end; i += 1) {
      ...
      lines.push(fit(`${prefix}${checked} ${selected ? t.fg("accent", name) : name} ${t.fg("muted", this.args.strategy.fieldTypeLabel(field))}`, width));
    }
    if (end < filtered.length) lines.push(t.fg("dim", `↓ ${filtered.length - end} more`));
  ...
  private renderQueryPane(width: number): string[] {
    ...
    for (const line of this.queryText.split("\n").slice(0, 12)) lines.push(fit(t.fg("toolOutput", line), width));
    if (this.queryText.split("\n").length > 12) lines.push(t.fg("dim", "… query truncated; press e to edit/view"));
    lines.push(fit(t.fg("muted", `w: ${this.args.strategy.whereLabel} · l: ${this.args.strategy.limitLabel} (${this.limit}) · e: edit full query · b: rebuild`), width));
    if (this.error) {
      ...
      lines.push("", t.fg("dim", "Press r to run. Press c to copy. Press S after running to save."));
    }
    return lines.map((line) => fit(line, width));
  }
  ...
  private renderResultTable(result: RunResult, width: number): string[] {
    ...
    const lines = [t.fg("success", `Returned ${result.totalReturned} row(s)`) + (result.rows.length ? t.fg("dim", " · ↑↓ row · enter detail") : "")];
  ...
  private renderRecordDetail(width: number): string[] {
    ...
    const labelW = Math.max(8, Math.min(34, names.reduce((acc, n) => Math.max(acc, visibleWidth(n)), 0)));
    ...
    for (const name of names) {
      ...
      for (let i = 1; i < pieces.length; i += 1) fieldLines.push(`${pad("", labelW)}  ${fit(pieces[i]!, valueW)}`);
    }
  ...
  private renderHelp(width: number): string[] {
    ...
    const lines = [
      ...
      "",
      t.fg("muted", "Compatibility aliases: uppercase L also edits limit; uppercase S also opens save."),
    ];
    return lines.map((line) => fit(line, width));
  }
  ...
  private handleSwitchMenuInput(data: string): void {
    ...
    else if (matchesKey(data, Key.down)) this.switchMenuCursor = Math.min(opts.length - 1, this.switchMenuCursor + 1);
  ...
  private previewState(): QueryBuildState<TObject> {
    return { selectedObject: this.selectedObject, selectedFieldNames: this.selectedFieldNames(), whereClause: this.whereClause, limit: this.limit };
  }
  ...
  private handleQueryEditInput(data: string): void {
    ...
    if (matchesKey(data, Key.left)) this.queryEditCursor = Math.max(0, this.queryEditCursor - 1);
    else if (matchesKey(data, Key.right)) this.queryEditCursor = Math.min(this.queryEditBuffer.length, this.queryEditCursor + 1);
      ...
      if (this.queryEditCursor < this.queryEditBuffer.length) {
        this.queryEditBuffer = this.queryEditBuffer.slice(0, this.queryEditCursor) + this.queryEditBuffer.slice(this.queryEditCursor + 1);
      }
    } else if (isBackspaceKey(data)) {
      if (this.queryEditCursor > 0) {
        this.queryEditBuffer = this.queryEditBuffer.slice(0, this.queryEditCursor - 1) + this.queryEditBuffer.slice(this.queryEditCursor);
        this.queryEditCursor -= 1;
      }
    } else if (data.length > 0 && !data.startsWith("\u001b")) {
      const printable = data.replace(/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/g, "");
      if (printable) {
        this.queryEditBuffer = this.queryEditBuffer.slice(0, this.queryEditCursor) + printable + this.queryEditBuffer.slice(this.queryEditCursor);
  ...
  private renderQueryEditor(width: number): string[] {
    ...
    const lines = [t.fg("accent", t.bold("Edit query in-place")), t.fg("dim", "enter save · esc cancel · arrows/home/end move · backspace/delete edit")];
    const rawLines = withCursor.split("\n");
    const cursorLine = before.split("\n").length - 1;
    const maxVisible = 18;
    const start = Math.max(0, Math.min(Math.max(0, rawLines.length - maxVisible), cursorLine - Math.floor(maxVisible / 2)));
  ...
  private async handleSaveMenuInput(data: string): Promise<void> {
    ...
    else if (matchesKey(data, Key.down)) this.saveMenuCursor = Math.min(this.saveOptions().length - 1, this.saveMenuCursor + 1);
    else if (matchesKey(data, Key.enter)) {
    ...
    } else if (data.toLowerCase() === "j") this.saveMenuCursor = Math.min(this.saveOptions().length - 1, this.saveMenuCursor + 1);
  ...
  private async saveSelectedFormat(format: ExportFormat): Promise<void> {
    ...
    try {
      const file = await saveResult({ cwd: this.args.cwd, result: this.result, baseName: this.args.strategy.exportBaseName(this.previewState()), format });
  ...
  private renderSaveMenu(width: number): string[] {
    ...
    if (this.result) {
      lines.push("", t.fg("muted", `Rows: ${this.result.totalReturned} · Destination: ${this.args.cwd}/.sf-data-explorer/exports/`));
    }
    return lines.map((line) => fit(line, width));
  }
  ...
  async selectObjectByName(name: string, force: boolean): Promise<void> {
    ...
    const index = this.objects.findIndex((obj) => this.args.strategy.objectName(obj).toLowerCase() === wanted);
    if (index < 0) {
  ...
  private async loadFields(obj: TObject, force: boolean): Promise<void> {
    ...
    try {
      ...
      this.selectedFields.clear();
      for (const fieldName of this.args.strategy.defaultFieldSelections(this.fields)) this.selectedFields.add(fieldName);
  ...
  private handleEditInput(data: string): void {
    ...
    if (matchesKey(data, Key.enter)) {
      if (this.editing === "where") {
        ...
        this.status = this.whereClause ? `${this.args.strategy.whereLabel} set.` : `${this.args.strategy.whereLabel} cleared.`;
  ...
  private handleDetailInput(data: string): void {
    ...
    if (data === "c") {
      ...
      if (row) this.args.done({ kind: "copyToEditor", text: JSON.stringify(row, null, 2), label: "record JSON" });
    }
  }
  ...
  private selectedFieldNames(): string[] {
    return this.fields.map((f) => this.args.strategy.fieldName(f)).filter((name) => this.selectedFields.has(name));
  }

  private filteredObjects(): TObject[] {
    const q = this.objectQuery.trim().toLowerCase();
    if (!q) return this.objects;
    return this.objects.filter((obj) => this.args.strategy.objectQueryHay(obj).toLowerCase().includes(q));
  }

  private filteredFields(): TField[] {
    const q = this.fieldQuery.trim().toLowerCase();
    if (!q) return this.fields;
    return this.fields.filter((field) => this.args.strategy.fieldQueryHay(field).toLowerCase().includes(q));
  }
  ...
  private applyExpansion(): void {
    if (this.expanded) {
      ...
      for (const j of others) this.paneWeights[j as 0 | 1 | 2] = 0.10;
    } else this.paneWeights = [0.30, 0.32, 0.38];
  }
