...
import { buildHelpText, DEFAULT_ORG, modeLabel, parseCommandArgs, type ParsedCommandArgs } from "../../lib/command.ts";
...
type CommandCtx = {
  ...
  ui: {
    ...
    custom: <T>(factory: (tui: { requestRender: () => void }, theme: any, keybindings: any, done: (value: T) => void) => any, options?: unknown) => Promise<T>;
...
export default function sfDataExplorer(pi: ExtensionAPI) {
  ...
  pi.registerCommand(COMMAND, {
    ...
    getArgumentCompletions: (prefix: string) => [
      ...
      "sosl refresh",
      "sql refresh",
    ].filter((v) => v.startsWith(prefix)).map((value) => ({ value, label: value })),
...
async function pickMode(ctx: CommandCtx): Promise<ExplorerMode | undefined> {
  const picked = await ctx.ui.select("SF Data Explorer mode", ["SOQL Explorer", "SOSL Explorer", "Data 360 SQL Explorer", "Cancel"]);
...
async function launchExplorer(pi: ExtensionAPI, ctx: CommandCtx, parsed: ParsedCommandArgs & { mode: ExplorerMode }): Promise<ExplorerSpaResult> {
...
async function buildInitialStrategy(
  ...
  if (parsed.mode === "soql") {
    const empty = createSoqlStrategy({ transport, org: parsed.org, initial: { objects: [], cacheLine: "Loading SOQL catalog…" } });
    const loaded = await runWithLoader(ctx, `${parsed.forceRefresh ? "Refreshing" : "Loading"} SOQL sObject catalog for ${parsed.org}…`, () => empty.loadCatalog(parsed.forceRefresh));
    if (!loaded) return undefined;
    const cacheLine = cacheStatus(loaded.kindLabel, loaded.cached, loaded.loadedAt);
    ctx.ui.notify(cacheLine, "info");
    return createSoqlStrategy({ transport, org: parsed.org, initial: { objects: loaded.value as CoreSObjectMeta[], cacheLine } });
  }
  if (parsed.mode === "sosl") {
    const empty = createSoslStrategy({ transport, org: parsed.org, initial: { objects: [], cacheLine: "Loading SOSL catalog…" } });
    const loaded = await runWithLoader(ctx, `${parsed.forceRefresh ? "Refreshing" : "Loading"} SOSL searchable catalog for ${parsed.org}…`, () => empty.loadCatalog(parsed.forceRefresh));
    if (!loaded) return undefined;
    const cacheLine = cacheStatus(loaded.kindLabel, loaded.cached, loaded.loadedAt);
    ctx.ui.notify(cacheLine, "info");
    return createSoslStrategy({ transport, org: parsed.org, initial: { objects: loaded.value as CoreSObjectMeta[], cacheLine } });
  }
  const empty = createData360SqlStrategy({ transport, org: parsed.org, initial: { objects: [], cacheLine: "Loading Data 360 catalog…" }, requestRender: () => {} });
  const loaded = await runWithLoader(ctx, `${parsed.forceRefresh ? "Refreshing" : "Loading"} Data 360 DMO+DLO catalog for ${parsed.org}…`, () => empty.loadCatalog(parsed.forceRefresh));
  if (!loaded) return undefined;
  const cacheLine = cacheStatus(loaded.kindLabel, loaded.cached, loaded.loadedAt);
  ctx.ui.notify(cacheLine, "info");
  return createData360SqlStrategy({ transport, org: parsed.org, initial: { objects: loaded.value as Data360ObjectMeta[], cacheLine }, requestRender: () => {} });
}

async function runWithLoader<T>(ctx: CommandCtx, label: string, work: (signal: AbortSignal) => Promise<T>): Promise<T | undefined> {
  const result = await ctx.ui.custom<T | { error: string } | null>((tui, theme, _kb, done) => {
    ...
    work(loader.signal).then(done).catch((error: unknown) => done({ error: error instanceof Error ? error.message : String(error) }));
    return loader;
  });
  if (result === null) {
