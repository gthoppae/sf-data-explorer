...
describe("query validators", () => {
  it("accepts read-only SELECT", () => {
    ...
    expect(validateSelectOnly("/* comment */ SELECT * FROM ssot__Individual__dlm LIMIT 5", "Data 360 SQL").ok).toBe(true);
  });

  it("rejects non-SELECT and mutation tokens", () => {
    expect(validateSelectOnly("DELETE FROM Account", "SOQL").ok).toBe(false);
    expect(validateSelectOnly("SELECT Id FROM Account; DELETE FROM Account", "SOQL").ok).toBe(false);
  });
