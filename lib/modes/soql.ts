...
export function defaultCoreFields(fields: CoreFieldMeta[]): string[] {
  ...
  const preferred = ["Id", "Name", "FirstName", "LastName", "Email", "Phone", "CreatedDate", "LastModifiedDate"];
...
export function createSoqlStrategy(args: {
  ...
  return {
    ...
    loadCatalog: async (force): Promise<CatalogLoad<CoreSObjectMeta>> => {
      ...
      if (cached) return { value: cached.value, cached: true, loadedAt: cached.loadedAt, kindLabel: "SOQL sObject catalog" };
      ...
      return { value: stored.value, cached: false, loadedAt: stored.loadedAt, kindLabel: "SOQL sObject catalog" };
    },
    ...
    objectSubtitle: (o) => [cleanLabel(o.label, o.name), o.custom ? "Custom" : "Standard", o.searchable ? "searchable" : undefined].filter(Boolean).join(" · "),
    objectQueryHay: (o) => `${o.name} ${o.label} ${o.labelPlural ?? ""} ${o.custom ? "custom" : "standard"} ${o.searchable ? "searchable" : ""}`,
    ...
    fieldQueryHay: (f) => `${f.name} ${f.label} ${f.type} ${f.custom ? "custom" : "standard"} ${f.filterable ? "filterable" : ""}`,
    fieldTypeLabel: (f) => `${f.type}${f.filterable ? " · filter" : ""}${f.sortable ? " · sort" : ""}`,
    ...
    runQuery: async (queryText, signal) => {
      ...
      return normalizeCoreQueryResult(resp.body, { query: queryText, targetOrg: resp.context.targetOrg ?? org, apiVersion: resp.context.apiVersion });
    },
