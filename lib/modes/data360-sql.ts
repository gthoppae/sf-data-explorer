...
interface MetadataResponse { metadata?: Data360ObjectMeta[] }
...
function unwrapMetadataDetail(raw: MetadataDetailResponse): MetadataDetailResponse {
  if (Array.isArray(raw.metadata) && raw.metadata.length === 1) return raw.metadata[0] as MetadataDetailResponse;
  if (Array.isArray(raw.dataModelObject) && raw.dataModelObject.length === 1) return raw.dataModelObject[0] as MetadataDetailResponse;
  if (Array.isArray(raw.dataLakeObjects) && raw.dataLakeObjects.length === 1) return raw.dataLakeObjects[0] as MetadataDetailResponse;
  if (Array.isArray(raw.items) && raw.items.length === 1) return raw.items[0] as MetadataDetailResponse;
  return raw;
}
...
function extractMetadataFields(raw: MetadataDetailResponse): Data360FieldMeta[] {
  ...
  const fields = candidates.find((value): value is Data360FieldMeta[] => Array.isArray(value)) ?? [];
  return fields.filter((field) => field.name).map((field) => ({
    name: field.name,
...
export function buildData360Sql(state: QueryBuildState<Data360ObjectMeta>): string {
  ...
  const projection = state.selectedFieldNames.length ? state.selectedFieldNames.map(quoteData360Identifier).join(",\n  ") : "*";
...
export function defaultData360Fields(fields: Data360FieldMeta[]): string[] {
  ...
  const selected = scored.filter((s) => s.score > 0).slice(0, 6).map((s) => s.name);
...
export function createData360SqlStrategy(args: {
  ...
  const applyFilter = (objects: Data360ObjectMeta[]) => filter === "All" ? objects : objects.filter((o) => o.entityType === filter);
  const nextFilter = () => { filter = filter === "All" ? "DMO" : filter === "DMO" ? "DLO" : "All"; };
  const kindLabel = () => filter === "All" ? "DMO+DLO" : filter;

  async function loadCombined(force: boolean): Promise<{ value: Data360ObjectMeta[]; cached: boolean; loadedAt: number }> {
    ...
    const [dmo, dlo] = await Promise.all([
      transport.callRest<MetadataResponse>({ targetOrg: org, method: "GET", path: "/ssot/metadata-entities", query: { entityType: "DataModelObject" } }),
      transport.callRest<MetadataResponse>({ targetOrg: org, method: "GET", path: "/ssot/metadata-entities", query: { entityType: "DataLakeObject" } }),
    ]);
    const value = [
      ...(dmo.body.metadata ?? []).filter((m) => m.name).map((m) => ({ ...m, entityType: "DMO" as const })),
      ...(dlo.body.metadata ?? []).filter((m) => m.name).map((m) => ({ ...m, entityType: "DLO" as const })),
  ...
  return {
    ...
    loadCatalog: async (force): Promise<CatalogLoad<Data360ObjectMeta>> => {
      ...
      return { value: applyFilter(loaded.value), cached: loaded.cached, loadedAt: loaded.loadedAt, kindLabel: `${kindLabel()} catalog` };
    },
    loadFields: async (obj, force): Promise<FieldsLoad<Data360FieldMeta>> => {
      ...
      if (cached) return { value: cached.value, cached: true, loadedAt: cached.loadedAt, kindLabel: `${obj.name} queryable fields` };
      ...
      if (typeof resp.body.errorCode === "string") {
        throw new Error(`${resp.body.errorCode}: ${String(resp.body.message ?? "Could not load Data 360 metadata")}`);
      }
      const value = extractMetadataFields(resp.body);
      const stored = setCached(key, value);
      return { value: stored.value, cached: false, loadedAt: stored.loadedAt, kindLabel: `${obj.name} queryable fields` };
    },
    objectName: (o) => o.name ?? "",
    ...
    runQuery: async (queryText, signal) => {
      ...
      return normalizeData360SqlResult(resp.body as Data360SqlResponse, { query: queryText, targetOrg: resp.context.targetOrg ?? org, apiVersion: resp.context.apiVersion });
    },
