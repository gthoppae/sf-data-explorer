...
import { normalizeCoreQueryResult, normalizeCoreSearchResult, normalizeData360SqlResult } from "../lib/result-normalize.ts";
...
describe("result normalization", () => {
  it("normalizes SOQL records and strips attributes from table rows", () => {
    ...
    }, { query: "SELECT Id, Name FROM Account LIMIT 1", targetOrg: "wh", apiVersion: "66.0" });
  ...
  it("normalizes Data 360 array rows", () => {
    const result = normalizeData360SqlResult({
      ...
      data: [["1", "Ada"]],
      returnedRows: 1,
    }, { query: "SELECT id, name FROM x LIMIT 1", targetOrg: "wh" });
